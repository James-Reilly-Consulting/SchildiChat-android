    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (headerPos != RecyclerView.NO_POSITION) {
            ...
            if (childInContact != null && isHeader(parent.getChildAdapterPosition(childInContact))) {
                updateOverlaidHeaders(parent, headerPos, true)
            ...
            updateOverlaidHeaders(parent, overlaidHeaderPos, false)
            drawHeader(c, currentHeader)
        } else {
            // Show hidden header again
            updateOverlaidHeaders(parent, null, false)
        }
    }

    private fun updateOverlaidHeaders(parent: RecyclerView, headerPos: Int?, inContact: Boolean) {
        if (lastHeaderPos != headerPos) {
            // Show hidden header again
            lastHeaderPos?.let {
                updateOverlaidHeader(parent, it, false)
            }
            // Remember new hidden header. While in contact, do not hide the header
            lastHeaderPos = if (inContact || headerPos == null) {
                null
            } else {
                if (updateOverlaidHeader(parent, headerPos, true)) {
                    headerPos
                } else {
                    null
                }
